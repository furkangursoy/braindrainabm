...
to setup
  ...
  set numberOfTurtles 1000
  set numberOfLinks numberOfTurtles * linksToNodesRatio

  create 1000

  if (networktype = "erdos-renyi")
  [
    erdosrenyi numberOfLinks
  ]
  if (networktype = "barabasi-albert")
  [
    barabasialbert numberOfLinks
  ]
  reset-ticks
end
...
to birth  ;Section 7 - Paragraph 1
  ask n-of 33 turtles with [ s = 1 ][hatch 1 [
    ...
    setxy random-xcor random-ycor
    if networktype = "erdos-renyi" [erdosrenyiBirth random (4 * linksToNodesRatio + 1) ]
    if networktype = "barabasi-albert" [barabasialbertBirth random (4 * linksToNodesRatio + 1) ]
    ]
  ]
end

to erdosrenyi [nl]
  while [ count links < nl] [
    ask one-of turtles [
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [s = 1]"
